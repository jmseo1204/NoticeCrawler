...
def SNU_ECE_JOB_OPENINGS_crawl(driver, bot, conf):

    MAX_SEARCH_NUM = 5
    searching_keywords = []
    search_query = ""
    page_num = 1
    page_link = "https://ece.snu.ac.kr/community/job-openings?page=1"
    feed_template = (
        "[SNU ECE 진로 공지사항] \n\n"
        + (f"키워드 {searching_keywords}에 해당하는 " if searching_keywords != [] else "")
        + "업데이트 게시글은 다음과 같습니다. \n\n"
    )
    sent_titles_file_loc = os.path.join(os.path.dirname(__file__), "src/SNU_ECE_JOB_OPENINGS_sent_titles.pkl")
    ...
    while i < MAX_SEARCH_NUM + 1:
        try:
            title_loc = BASE_LOC + f"/tr[{block_ptr}]/td[3]/a/span"
            title = driver.find_element(By.XPATH, title_loc).text

            href_loc = BASE_LOC + f"/tr[{block_ptr}]/td[3]/a"
            href = driver.find_element(By.XPATH, href_loc).get_attribute("href")

            date_loc = BASE_LOC + f"/tr[{block_ptr}]/td[4]"
